...
tspan = [0:p.dt:10];
delta = 0.02; % Verzögerung durch Berechnung
N=10;   % Anzahl prädizierter Schritte
...
%% Eingangsvektor
u0=zeros(1,N+1)   % Startwert für die erste Schätzung
%%
...
% [u_opt, fehlersum_opt] = fminunc(@(u) kosten(u, tspan, x0, x3_soll, p), u0);
% 
% %% Optimum plotten
% [t_opt, x_opt] = ode45(@nmpcPendelDGS, tspan, x0, odeset, u_opt, p);
% 
% figure(2)
% subplot(2,1,1)
% plot(t_opt,u_opt);
...
i
    %% Optimierung
    [u_opt, fehlersum_opt] = fminunc(@(u) kosten(u, tspan(1:1+N), x_mess(end,:), x3_soll, p), u0);
